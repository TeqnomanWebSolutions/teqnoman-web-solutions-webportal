        ...
        <div className="invoice-container" id="invoice-container">
          <div className="main-header">
            ...
            <div className="info-section">
              ...
              <div className="text">
                ...
                <address>103, Elite Luxuria,<br />Samarpan Circle,<br />Jamnagar, Gujarat - 361006<br />Phone: +91 8758431673</address>
              </div>
            </div>
          </div>
          ...
          <div className="invoice-header">
            <div className="company-section">
              ...
              {jsonData.gst_number && <address>GSTIN: {jsonData.gst_number}</address>}
            </div>
          ...
          <div className="table-section">
            ...
            <table>
              ...
              <tbody>
                {jsonData.invoice_type == 'custom' && jsonData?.invoice_details?.map((d, index) => {
                  return (<>
                    <tr>
                      <td>{index + 1}</td>
                      <td>{d.description}</td>
                      <td>₹
                        {jsonData.gst_calculation == "inclusive_gst" && (d.price - d.price * 0.18).toLocaleString('en-IN')}
                        {jsonData.gst_calculation == "exclusive_gst" && (d.price).toLocaleString('en-IN')}
                      </td>
                    </tr>
                  </>)
                })}
                {jsonData.invoice_type == 'fixed' && <tr>
                  <td>1</td>
                  <td>{jsonData.project_description}</td>
                  <td>₹
                    {jsonData.gst_calculation == "inclusive_gst" && (jsonData.subtotal - jsonData.subtotal * 0.18).toLocaleString('en-IN')}
                    {jsonData.gst_calculation == "exclusive_gst" && (jsonData.subtotal).toLocaleString('en-IN')}
                  </td>
                </tr>}
                <tr>
                  <td></td>
          ...
          <div className="payment-section">
            ...
            <div className="grand-total">
              <div className='gst-card'>
                {jsonData.state == "Gujarat" && <><div className="gst-data">
                  <p className="text">CGST 9%</p>
                  <p className="value">₹{(jsonData.subtotal * 0.18 / 2).toLocaleString('en-IN')}</p>
                </div>
                ...
                {jsonData.state != "Gujarat" && <div className="gst-data">
                  <p className="text">IGST 18%</p>
                  <p className="value">₹{(jsonData.subtotal * 0.18).toLocaleString('en-IN')}</p>
                </div>}
              </div>
              <div className="grand-total-card">
                <p className="text">Grand Total</p>
                <p className="value">₹
                  {jsonData.gst_calculation == 'inclusive_gst' && (jsonData.subtotal).toLocaleString('en-IN')}
                  {jsonData.gst_calculation == 'exclusive_gst' && (jsonData.subtotal * 0.18 + jsonData.subtotal).toLocaleString('en-IN')}
                </p>
              </div>
