...
export async function getServerSideProps(context) {
    ...
    if (user_id) {
        try {
            const response = await instance.get(`${baseUrl}/api/projects?employee_id=${user_id}`);
            return { props: { projects: response.data.filter(d => d.is_deleted !== true) } };
        } catch (error) {
