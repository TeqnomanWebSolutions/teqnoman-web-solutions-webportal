...
export default function Profile() {
    ...
    const [initialValues, setInitialValues] = useState({
        main_personal_detail: null,
        main_qualification_detail: null,
        main_bank_detail: null,
        main_prev_detail: null,
    });
    useEffect(() => {
        if (user) {
    ...
    function getQualificationData() {
        instance.get(`/api/employee_qualification?id=${user?.id}`).then((res) => {
            if (res.data) {
                setInitialValues((prevData) => { return { ...prevData, main_qualification_detail: res.data } });
    ...
    async function getPrevData() {
        ...
        if (res.data) {
            setInitialValues((prevData) => { return { ...prevData, main_prev_detail: { is_fresher: res.data?.[0]?.is_fresher, prevData: [...res.data] } } });
    ...
    async function getAllBankInfo() {
        let res = await instance.get(`/api/employee_bank_info?id=${user.id}`);
        if (res.data) {
            setInitialValues((prevData) => { return { ...prevData, main_bank_detail: res.data } });
            bank_info.setValues(res.data);
        }
    }

    async function getPersonalInfo() {
        let res = await instance.get(`/api/employees?id=${user?.id}`);
        if (res.data) {
            personalInfoFormik.setValues({ ...res.data });
            setInitialValues((prevData) => { return { ...prevData, main_personal_detail: { ...res.data } } });
        ...
        onSubmit: async (values) => {
            ...
            if (user.id) {
                try {
                    ...
                    let changes = getDirtyValues(personal_detail, initialValues.main_personal_detail);
                    ...
                    if (fields.length != 0) {
                        fields.map((f) => {
                            let data = {
                                ...
                                oldValue: initialValues.main_personal_detail[f],
                                newValue: personal_detail[f],
                                user_id: user.id,
                                fieldName: f,
                                apiURL: `/api/employees?emp_id=${user.id}`,
                                created_at: new Date()
                            }
                            sendData.push(data);
                        })
                    }
                    let res = await instance.post(`/api/change_request`, sendData);
                    if (res.status === 200) {
                        toast.info("Your change requests have been submitted to the admin. They will be updated once approved by the admin.")
                    }
                    getPersonalInfo();
    ...
    const qualificationFormik = useFormik({
        ...
        onSubmit: async (values) => {
            if (values.qualification.length != 0) {
                values.qualification.map(async (d, index) => {
                    let post_id = d._id;
                    if (post_id) {
                        let changes = getDirtyValues(d, initialValues.main_qualification_detail[index]);
                        let fields = Object.keys(changes);
                        let sendData = [];
                        if (fields.length != 0) {
                            fields.map((f) => {
                                let data = {
                                    collectionName: "employee_qualification",
                                    oldValue: initialValues.main_qualification_detail[index][f],
                                    newValue: d[f],
                                    user_id: user.id,
                                    fieldName: f,
                                    apiURL: `/api/employee_qualification?id=${d._id}`,
                                    row_id: d._id,
                                    created_at: new Date()
                                }
                                sendData.push(data);
    ...
    const bank_info = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            let changes = getDirtyValues(bank_info, initialValues.main_bank_detail);
            let fields = Object.keys(changes);
            let sendData = [];
            if (fields.length != 0) {
                fields.map((f) => {
                    let data = {
                        collectionName: "employee_bank_info",
                        oldValue: initialValues.main_bank_detail[f],
                        newValue: bank_info[f],
                        user_id: user.id,
                        fieldName: f,
                        apiURL: `/api/employee_bank_info`,
                        created_at: new Date()
                    }
                    sendData.push(data);
                })
    ...
    const experience = useFormik({
        ...
        onSubmit: async (values) => {
            if (values.prevData.length != 0) {
                ...
                reqData.map(async (data, index) => {
                    let item = await data;
                    if (item._id) {
                        let id = item._id;
                        delete item._id;

                        let changes = getDirtyValues(item, initialValues.main_prev_detail.prevData[index]);
                        let fields = Object.keys(changes);
                        let sendData = [];
                        if (fields.length != 0) {
                            fields.map((f) => {
                                if (f != "experience_letter" && f != "salary_slip") {
                                    let data = {
                                        collectionName: "employment_history",
                                        oldValue: initialValues.main_prev_detail.prevData[index][f],
                                        newValue: item[f],
                                        user_id: user.id,
                                        fieldName: f,
                                        apiURL: `/api/employment_history?id=${id}`,
                                        row_id: id,
                                        created_at: new Date()
                                    }
                                    sendData.push(data);
                        ...
                        // await instance.put(`/api/employment_history?id=${id}`, item).then((res) => {
                        //     if (res.status === 200) {
            ...
            {loading ? (<div className="spinner-container">
                ...
                <Tabs>
                        ...
                        <TabPanel>
                            <div className={style.borderBox}>
                                <div className={style.profileImage}>
                                    <div className={style.profileInfo}>
                                        <img src={profileImg || "/images/dummy-profile.png"} width={100} height={100} alt="profile" className={style.profileImg} onError={(e) => {
                                            e.target.src = `https://placehold.co/100x100/ff701f/fff?text=${personalInfoFormik.values.first_name.charAt(0).toUpperCase()}${personalInfoFormik.values.last_name.charAt(0).toUpperCase()}`;
                                        }} />
                                ...
                                <div className='row'>
                                    ...
                                    <div className="col-lg-4 col-sm-6">
                                        <div className="form_control">
                                            <label htmlFor='contact' className="custom-label">Contact No.</label>
                                            <input
                                                name="contact_number"
                                                onChange={personalInfoFormik.handleChange}
                                                onBlur={personalInfoFormik.handleBlur}
                                    ...
                                    <div className="col-lg-4 col-sm-6">
                                        <div className="form_control">
                                            <label htmlFor='medical_issue' className="custom-label">Any Medical Issue?</label>
                                            <input
                    ...
                    <form onSubmit={qualificationFormik.handleSubmit}>
                        <TabPanel>
                            <div className={style.borderBox}>
                                {qualificationFormik.values.qualification.map((q, index) =>
                                    <div className={`row ${style.newRow}`} key={index}>
                    ...
                    <form onSubmit={experience.handleSubmit}>
                        <TabPanel>
                                        ...
                                        <div className="col-lg-6 col-md-6">
                                            <div className="form_control">
                                                ...
                                                <div>
                                                    ...
                                                    {(data?.salary_slip?.name) && <div className={style.upload}><a href={showImage(data?.salary_slip)} target='_blank'><FileThumbnail name={data?.salary_slip?.name} /> {data?.salary_slip?.name}</a></div>}
                                                    <input
                                        ...
                                        <div className="col-lg-6 col-md-6">
                                            <div className="form_control">
                                                ...
                                                <div>
                                                    ...
                                                    {(data?.experience_letter?.name) && <div className={style.upload}><a href={showImage(data?.experience_letter)} target='_blank'><FileThumbnail name={data?.experience_letter?.name} /> {data?.experience_letter?.name}</a></div>}
                                                    <input
                            ...
                            <div className={style.save}>
                                <ButtonUi cssClass='btnuiRounded' text='Save' type='submit' callBack={experience.handleSubmit} />
                            </div>
                        </TabPanel>
                    </form>
                                ...
                                <div className={`row ${style.alignment}`}>
                                    <div className="col-lg-4">
                                        <div className="form_control">
                                            ...
                                            <label className={style.upload}>
                                                <a target="_blank" href={uploadedDocuments?.aadhar?.url}>
                                                    <FileThumbnail name={uploadedDocuments?.aadhar?.filename} /> {uploadedDocuments?.aadhar?.filename}
                                                </a>
                                            </label>
                                        </div>
                                    </div>
                                    <div className="col-lg-4">
                                        <div className="form_control">
                                            ...
                                            <label className={style.upload}>
                                                <a target="_blank" href={uploadedDocuments?.PAN?.url}>
                                                    <FileThumbnail name={uploadedDocuments?.PAN?.filename} /> {uploadedDocuments?.PAN?.filename}
                                                </a>
                                            </label>
                                        </div>
                                    </div>
                                    {uploadedDocuments?.resume && <div className="col-lg-4">
                                        <div className="form_control">
                                            <label className="custom-label ">Resume</label>
                                            <label className={style.upload}>
                                                <a target="_blank" href={uploadedDocuments?.resume?.url}>
                                                    <FileThumbnail name={uploadedDocuments?.resume?.filename} /> {uploadedDocuments?.resume?.filename}
                                                </a>
                                            </label>
                                        </div>
                                    </div>}
                                    {uploadedDocuments?.certificate && <div className="col-lg-4">
                                        <div className="form_control">
                                            <label className="custom-label ">Achievement Certificate</label>
                                            <label className={style.upload}>
                                                <a target="_blank" href={uploadedDocuments?.certificate?.url}>
                                                    <FileThumbnail name={uploadedDocuments?.certificate?.filename} /> {uploadedDocuments?.certificate?.filename}
                                                </a>
                                            </label>
                                        </div>
                                    </div>}
